...
def load_trigger_pairs(path, map_location):
    """Load the authors' pickle on either CUDA or CPU hosts.

    The trigger file contains tensors serialized through ``torch.save``.
    The published artifact was written on CUDA, so plain ``pickle.load``
    fails on a CPU-only workstation.  Temporarily supply ``map_location``
    for those embedded tensor loads without changing the stored artifact.
    """
    original_torch_load = torch.load

    def torch_load_with_location(*args, **kwargs):
        kwargs.setdefault('map_location', map_location)
        return original_torch_load(*args, **kwargs)

    try:
        torch.load = torch_load_with_location
...
if __name__ == "__main__":
    ...
    for model_name in model_filename_set:
        ...
        model.load_state_dict(
            torch.load(model_dir+model_name, map_location=device, weights_only=True)
        )
        ...
        neuron_trigger_pair = load_trigger_pairs(
            model_dir+model_name[:-4]+"_neuron_trigger_pair.pkl", device
        )
        ...
        for backdoor_model_name in backdoor_model_filename_set:
            ...
            backdoor_model.load_state_dict(
                torch.load(
                    backdoor_model_dir+backdoor_model_name,
                    map_location=device,
                    weights_only=True,
                )
            )
