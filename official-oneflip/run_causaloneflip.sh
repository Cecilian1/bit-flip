...
base=(uv run --frozen python ./causal_oneflip.py --method causal --dataset CIFAR10 --clean-checkpoint ./saved_model/resnet_CIFAR10/clean_model_1.pth --data-root ../dataset/CIFAR10 --device "$device" --target-classes 6 --calibration-size 1024 --optimization-size 768 --optimization-batch-size 128 --test-batch-size 512 --workers 2)
...
PY
  ...
  select-params)
    ...
    uv run --frozen python ./select_causaloneflip_params.py --results ./runs/causaloneflip-tune-class6-fine-1/results.csv --results ./runs/causaloneflip-tune-class6-fine-2/results.csv --results ./runs/causaloneflip-tune-class6-fine-3/results.csv --output-dir ./runs/causaloneflip-tune-class6
    ;;
  *) echo "Usage: $0 {tune-coarse|tune-fine|select-params} [--resume]" >&2; exit 2 ;;
esac
